...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # [128,32,768]        [128,768]
        # 前者为句子的ids对应的输出，当output_all_encoded_layers = False仅为最后一层隐藏层的输出   #后者为[cls]对应的输出
        encoder_out, pooled = self.bert(context, attention_mask = mask, output_all_encoded_layers = False) #shape [128,32,768]  [128,768]
        # [128,768]
        #out[128,1,32,768]
        out = encoder_out.unsqueeze(1) #扩展一个通道维数，否则进不了卷积层，具体讲解见61课
